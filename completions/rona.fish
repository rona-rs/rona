...
complete -c rona -n "__fish_rona_using_subcommand help; and not __fish_seen_subcommand_from add-with-exclude commit completion config generate init list-status push set-editor sync help" -f -a "config" -d 'Manage configuration files (create or edit local or global config)'
