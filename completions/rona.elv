...
set edit:completion:arg-completer[rona] = {|@words|
    ...
    var completions = [
        &'rona'= {
            ...
            cand pr 'Open a pull request (or merge request) for the current branch'
            cand mr 'Open a pull request (or merge request) for the current branch'
            cand pull-request 'Open a pull request (or merge request) for the current branch'
        ...
        &'rona;pr'= {
            cand -t 'Branch to target (defaults to `pr_target`, then the remote''s default branch)'
            cand --target 'Branch to target (defaults to `pr_target`, then the remote''s default branch)'
            cand -T 'Title of the request (overrides the document heading)'
            cand --title 'Title of the request (overrides the document heading)'
            cand -b 'Markdown file holding the whole request (skips the editor)'
            cand --body-file 'Markdown file holding the whole request (skips the editor)'
            cand -l 'Label to apply (repeat for several)'
            cand --label 'Label to apply (repeat for several)'
            cand -r 'Reviewer to request (repeat for several)'
            cand --reviewer 'Reviewer to request (repeat for several)'
            cand -A 'Assignee to set (repeat for several)'
            cand --assignee 'Assignee to set (repeat for several)'
            cand --backend 'Backend used to open the request'
            cand --remote 'Remote to open the request against (defaults to `pr_remote`, then `origin`)'
            cand -f 'Config file to use instead of the default global/project hierarchy'
            cand --config-file 'Config file to use instead of the default global/project hierarchy'
            cand -d 'Open the request as a draft'
            cand --draft 'Open the request as a draft'
            cand -w 'Open the pre-filled web form instead of using a CLI backend'
            cand --web 'Open the pre-filled web form instead of using a CLI backend'
            cand --no-edit 'Use the request document as it is instead of opening the editor'
            cand --no-push 'Do not push the source branch before opening the request'
            cand -y 'Skip the confirmation prompt'
            cand --yes 'Skip the confirmation prompt'
            cand --dry-run 'Show what would be opened without opening anything'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'rona;mr'= {
